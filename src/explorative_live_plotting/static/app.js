...
function ensureConfig() {
  ...
  config.layers.forEach(layer => {
    layer.options ??= {};
    layer.time_bin_start_by ??= layer.options.start_by ?? 'monday';
    layer.break_on_missing_time_bin ??= false;
    delete layer.options.start_by;
  });
  config.annotations ??= [];
...
function newLayer() {
  ...
  return {
    ...
    time_bin: null, time_bin_start_by: 'monday', break_on_missing_time_bin: false, filter_logic: 'and', required_filters: [], filters: [],
...
function renderLayers() {
  ...
  config.layers.forEach((layer, index) => {
    ...
    layer.time_bin_start_by ??= 'monday';
    layer.break_on_missing_time_bin ??= false;
    ...
    if (layer.break_on_missing_time_bin && layer.time_bin) advancedStatus.push('time gaps');
    ...
    q('.break-on-missing-time-bin').onchange = event => { layer.break_on_missing_time_bin = event.target.checked; changed(false); };
